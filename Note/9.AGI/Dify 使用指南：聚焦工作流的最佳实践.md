...
## 知识流水线
知识流水线（Knowledge Pipeline）是 Dify 平台中用于自动化处理上传文件的一套流程。它会把原始文件（比如 PDF、Word、图片等）经过一系列步骤，比如解析、分段、抽取标题和摘要、分层分块等，最终转化为结构化的知识数据，方便后续检索和智能生成。每个处理步骤都是一个节点，常见节点有文件输入、文件解析、文本分块、知识库入库等，整个流程可以自定义和组合，实现复杂的数据处理和知识管理需求。

知识流水线的主要功能包括：自动解析文件内容、分段处理文本、抽取关键信息、优化知识库结构，支持知识库的构建和维护，提高知识检索和问答的效果。你可以通过 Web UI 或 API 上传文件，系统会自动进入流水线处理流程。需要注意的是，通过 API 上传时只会走默认的解析和分段逻辑，不会自动触发自定义流水线（比如特殊节点或自定义处理），只有通过 Web UI 上传才能保证完整执行自定义流程。未来 Dify 官方计划推出一键触发流水线的 API（如 create-by-pipeline），但目前还未上线。

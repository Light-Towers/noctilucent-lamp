...
> **📄 文档说明**：本技术文档内容结合了官方文档与内部实践经验。由于 Apache Doris 版本更新较快，部分细节（如具体工具类名、实验性参数等）可能随版本变动。**建议在执行关键操作前，务必以对应版本的[官方文档](https://doris.apache.org/docs/dev/get-starting/)为准。**
...
- **索引设计建议**
  - **多列过滤加速**：对于需要多列组合过滤加速的场景，建议通过合理排布**前缀索引**（Prefix Index）的列顺序，或者针对目标列建立**倒排索引**来实现。
  - **空间数据处理**：在处理地理信息系统（GEO）数据时，Doris 倾向于使用基于 Hilbert 曲线的 **S2 Geometry** 库（具有更好的空间连续性）。
...
Apache Doris 的高性能得益于其底层集成的多种优化算法。以下按功能分类介绍其主要算法及技术原理。

### 1. 存储与索引算法
Doris 采用列式存储与多级索引体系，以优化数据布局并加速查询。

- **列式存储**：数据按列组织，天然适合向量化计算。通过高压缩率减少物理存储需求，并显著提升 I/O 效率。
- **B+ 树索引**：传统的磁盘友好索引结构，用于支持高效的等值查询与范围查询。
- **LSM 树索引**：适用于写入密集型场景，通过追加写（Append-only）与定期合并（Compaction）平衡读写性能。
- **Bitmap 索引**：利用位运算高效完成多条件过滤，特别适用于高选择性（High Cardinality）列。
- **Zone Map**：在元数据中维护每列的最小值（Min）与最大值（Max），用于在扫描前进行数据块裁剪。
- **Bloom Filter / Cuckoo Filter / Xor Filter**：作为概率性索引，快速判断数据是否可能存在，减少无效的 IO 扫描。
- **HNSW（Hierarchical Navigable Small World）**：用于向量检索的近似最近邻（ANN）算法，支持高召回、低延迟的向量相似度搜索。
    > **💡 深入学习**：关于 HNSW 索引的底层原理、详细配置与 ANN 检索技巧，请参考 [HNSW 与 ANN 详解.md](./HNSW%20与%20ANN%20详解.md)。

### 2. 查询执行与优化算法
Doris 结合了先进的优化框架与向量化执行引擎。

- **向量化计算**：将数据按列批量处理，利用 CPU SIMD 指令加速，大幅提高 CPU 利用率。
- **RBO（基于规则的优化）**：通过预定义规则（如列裁剪、谓词下推、分区裁剪）改写查询计划，快速消除无效计算。
- **CBO（基于成本的优化）**：基于 Cascades 框架，利用统计信息与代价模型（考虑 IO、CPU、网络等）选择最优执行计划。
- **谓词下推/投影下推**：尽早过滤数据并仅读取必要列，减少中间结果集的大小与数据传输开销。
- **分片合并**：在分布式查询中高效合并多个分片结果，降低跨节点数据交换量。

### 3. 数据裁剪与过滤算法
在运行时通过多种裁剪策略减少扫描的数据规模。

- **静态裁剪**：根据分区列、Key 列的谓词直接跳过不需要的分区或 Segment。
- **动态裁剪**：在运行时根据已获取的数据（如 Join 的 Build 侧结果）动态生成过滤条件。
- **LIMIT/TopK 裁剪**：在满足 LIMIT 条件或排序截断点时提前终止读取，避免冗余 I/O。
- **JOIN 裁剪**：通过运行时过滤器（Runtime Filter）在 Probe 侧实施动态过滤。

### 4. 混合检索算法
支持关键词匹配与语义搜索的融合。

- **BM25**：全文检索的相关性评分算法，衡量关键词与文档的相关性。
- **向量相似度计算**：支持余弦相似度等度量，配合 HNSW 实现语义搜索。
- **结果融合算法**：使用 **RRF（Reciprocal Rank Fusion）** 或 **CC（Convex Combination）** 融合全文检索与向量检索的结果，提升搜索质量。

### 5. 高并发点查优化算法
针对超高并发的点查场景进行了深度优化。

- **LRU Cache**：维护行存缓存（Row Cache），提高热点数据的命中率。
- **短路径优化（Short‑Circuit）**：为点查生成轻量级计划，绕过完整的 MPP 调度与规划开销。
- **预处理语句**：缓存 SQL 解析结果与表达式，减少重复解析的 CPU 开销。

### 6. 分布式与资源调度算法
- **MPP（大规模并行处理）**：将查询拆分为多个任务在集群节点上并行执行，支持线性扩展。
- **负载均衡与数据分片**：自动分配任务并分布数据，避免单点瓶颈，增强系统容错。
- **动态资源调整**：根据系统负载动态优化资源分配，提高整体吞吐。
...
- **FE**：建议 16 核 64GB 以上。建议预留 100GB 以上的存储空间，使用 SSD 硬盘。。
- **BE**：建议 16 核 64GB 以上。存储空间按数据量 × 3（副本）计算，并额外预留 40% 用于压缩和中间数据。
...
> **🔔 实践说明**：以下展示的 Jar 包版本、类名及命令行参数均基于 **Flink 官方 CDC 文档**与 **Doris Flink Connector 文档**的组合实践示例，建议在使用时根据具体版本环境进行微调。
...
> 2. **逐级平滑迁移**：建议遵循 `2.x -> 3.0 -> 3.1 -> 3.2 -> ...` 的平滑执行路径。*注意：具体升级路径请参考各版本的 Release Notes。*

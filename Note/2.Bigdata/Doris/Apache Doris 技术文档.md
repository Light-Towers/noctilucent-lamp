...
5. **BE 节点下线操作（重要安全提示）**：
   ...
   **⚠️ 危险操作警告**：
   ...
   当需要从集群中移除 BE 节点时，不要使用 `DROP BACKEND` 命令，因为这会**立即删除 BE 节点且不会迁移其上的数据到其他 BE**，可能导致数据永久丢失。
   
   - **错误做法**（Doris 会拒绝执行并给出"强提醒"）：
     ```sql
   ...
   - **如果真的确认要强制删除**（不推荐，仅用于紧急情况）：
     ...
     ```
     注意：这里需要将 `DROP` 改为 `DROPP`，Doris 才会执行删除操作。
   
   - **✅ 推荐做法（生产环境）**：使用 `DECOMMISSION BACKEND` 命令
     ```sql
     ALTER SYSTEM DECOMMISSION BACKEND "127.0.0.1:9050";
     ```
     该命令会：
     1. 先将该 BE 上的数据块迁移到其他 BE 节点
     2. 等待数据迁移完成后自动下线节点
     3. 确保数据不丢失，实现安全的节点缩容
     
     可通过 `SHOW PROC '/backends'\G` 查看迁移进度。

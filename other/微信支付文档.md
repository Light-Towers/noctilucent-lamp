...
调用服务方法创建订单:
```
com.test.service.impl.HomeOrderServiceImpl#contractorOrder
```

#### 1.2 调用支付接口
调用支付接口并返回支付二维码(codeUrl):
```
com.test.controller.PayController#unifiedOrder
com.test.service.impl.PayServiceImpl#wxUnifiedOrder
```

#### 1.3 支付回调处理
支付成功后的回调处理:
```
com.test.service.impl.HomeOrderServiceImpl#buildOrderPayCallback
```
...
初始化支付配置信息并返回机构配置的方法:
```
com.test.service.impl.PayServiceImpl#initWxPayServiceByOrgId
```

配置获取流程：
1. **优先获取机构个性化配置**
   - 从 `com.test.common.org.OrgConfigService#getOrgApiConfig` 获取机构配置
2. **降级到全局配置**
   - 如果机构个性化配置不完整或不存在，则通过 `com.test.common.feign.service.SysService#queryOrgConfigVoById` -> `com.test.controller.SysOrgConfigApiController#getOrDefault` 获取全局配置

### 4. 配置管理平台

进入 [运营管理平台](http://192.168.0.9/web) (用户名: admin、密码: 111111)
...
配置保存的入口方法:
```
com.test.controller.SysOrgConfigApiController#save
```
